...
export const InitiativeCard: FC<InitiativeCardProps> = ({
  ...
  const InitiativeCardClasses = CN(
    `initiative-card bg-white lg:px-[32px] lg:py-[32px] rounded-[12px] lg:w-[368px] h-full shadow-[0px_25px_50px_-12px_rgba(95,111,140,0.1)]`,
    className,
    {}
  )
    ...
    <div className={InitiativeCardClasses} {...restProps}>
      ...
      {btnProps && (
        <div className='pt-[16px]'>
          <Button {...restBtnProps}>{children || 'Sign Up'}</Button>
        </div>
      )}
    </div>
  )
