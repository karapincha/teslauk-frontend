  ...
  (
    ...
    const ButtonClasses = CN(
      ...
      className,
      {
        ...
        'bg-N-100 text-N-800 hover:bg-N-200': appearance === 'secondary' && view === 'solid',
    ...
    return (
      <button className={CN(ButtonClasses)} disabled={disabled} ref={ref} {...restProps}>
        {sup && (
          <span
            className={CN(
              'absolute right-[-12px] top-0 inline-flex h-[20px] px-[4px] min-w-[20px] items-center justify-center rounded-full bg-B-500 text-[10px] font-900 text-white'
            )}>
            {sup}
          </span>
        )}
